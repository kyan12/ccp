...
function loadWorkflow(repoPath: string): WorkflowDefinition | null {
    ...
    try {
      ...
      for (const line of yamlLines) {
        ...
        if (kvMatch) {
          ...
          } else if (val === 'true') (config as Record<string, unknown>)[currentKey] = true;
          else if (val === 'false') (config as Record<string, unknown>)[currentKey] = false;
          else if (/^\d+$/.test(val)) (config as Record<string, unknown>)[currentKey] = parseInt(val, 10);
          else (config as Record<string, unknown>)[currentKey] = val;
        } else if (currentKey && line.match(/^\s+-\s+(.+)$/)) {
          const item = line.match(/^\s+-\s+(.+)$/)![1];
          const arr = (config as Record<string, unknown>)[currentKey];
          if (!Array.isArray(arr)) (config as Record<string, unknown>)[currentKey] = [];
          ((config as Record<string, unknown>)[currentKey] as string[]).push(item);
        }
      }
  ...
  return { config, promptTemplate, path: workflowPath };
}

function renderWorkflowPrompt(template: string, packet: JobPacket): string {
  const vars: Record<string, string> = {
    ticket: packet.ticket_id || 'UNTRACKED',
    goal: packet.goal || 'No goal provided',
    constraints: (packet.constraints || []).map((c: string) => `- ${c}`).join('\n') || 'None',
    acceptance_criteria: (packet.acceptance_criteria || []).map((c: string) => `- ${c}`).join('\n') || 'None',
    verification_steps: (packet.verification_steps || []).map((c: string) => `- ${c}`).join('\n') || 'None',
    review_feedback: (packet.review_feedback || []).map((c: string) => `- ${c}`).join('\n') || 'None',
    branch: packet.working_branch || 'main',
    repo: packet.repoKey || packet.repo || 'unknown',
  };
...
function buildPrompt(packet: JobPacket): string {
  // Check for WORKFLOW.md in the repo
  if (packet.repo && fs.existsSync(packet.repo)) {
    const workflow = loadWorkflow(packet.repo);
    if (workflow && workflow.promptTemplate) {
      const rendered = renderWorkflowPrompt(workflow.promptTemplate, packet);
      if (workflow.config.constraints && Array.isArray(workflow.config.constraints)) {
        const extraConstraints = workflow.config.constraints.filter(
          (c: string) => !(packet.constraints || []).includes(c)
        );
